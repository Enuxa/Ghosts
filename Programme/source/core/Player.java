	...
	*/
			...
			if (position != null && book.requestInitialization(this, position)){//	Si le joueur a saisi une position et qu'elle est autorisée
				...
				if (type != null){//	Si le joueur a sélectionné un type
					...
					if (isGoodStr != null){//	Si le joueur a choisi si son fantôme était bon ou mauvais
						Ghost ghost = Game.getCurrent().getFactory().createGhost(type, isGoodStr.equals("Bon"), this);
	...
	private boolean isSureOfInitialization (Board board, RuleBook book, Interface inter, boolean cheatMode){
		while (book.isReady(this)){
			...
			if (ans != null){
				//	Si le joueur est sûr de sa configuration.
				if (ans.equals("Oui"))
					return true;
				//	Si le joueur n'est pas sûr de sa configuration.
				else{
					String position = inter.readPosition("Quel fantôme souhaitez-vous déplacer ? ");
					//	Si la position de départ est correcte
					if (position != null && board.getSquare(position) != null){
						...
						if (position2 != null && board.getSquare(position2) != null){
							//	Si la position d'arrivée est correcte
							Square s2 = board.getSquare(position2);
							Ghost g2 = s2.getGhost();
							//	Si ces fantômes (s'ils existent) appartiennent au joueur.
							if (g1 != null && this.hasGhost(g1) && g2 != null && this.hasGhost(g2)){
								s.removeGhost();
								s2.removeGhost();
								s.putGhost(g2);
								s2.putGhost(g1);
